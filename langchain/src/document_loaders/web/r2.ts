...
/**
 * Represents the parameters for the R2Loader class. It includes
 * properties such as the S3 bucket, key, region, endpoint, unstructured API URL,
 * unstructured API key, S3 configuration, file system module, and
 * UnstructuredLoader module.
 */
export interface R2LoaderParams {
  bucket: string;
  key: string;
 ...
 * document loader for loading files from an R2 bucket.
 */
export class R2Loader extends BaseDocumentLoader {
  ...
  /**
   * Loads the file from the R2 bucket, saves it to a temporary directory,
  ...
  public async load() {
    const tempDir = this._fs.mkdtempSync(
      path.join(os.tmpdir(), "r2fileloader-")
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to download file ${this.key} from R2 bucket ${this.bucket}: ${e.message}`
      );
    }
